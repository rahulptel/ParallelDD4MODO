...
void expand_layer_topdown_cpu_kernel1(BDD* bdd,
                                     const int layer,
...
bool expand_layer_topdown_cpu_kernel3(BDD* bdd,
                                     const int layer,
                                     const bool maximization,
                                     ParetoFrontierManager* mgmr,
                                     const bool parallel_mode,
    ...
    for (int i = 0; i < layer_size; ++i) {
        ...
        for (std::vector<Node*>::iterator prev = node->prev[first_arc_type].begin();
            ...
            const std::vector<ObjType>& prev_sols = (*prev)->pareto_frontier->sols;
...
void expand_layer_topdown_cpu_kernel1_mdd(MDD* mdd,
                                          const int l,
                                          ParetoFrontierManager* mgmr,
                                          const bool parallel_mode,
                                          const int threads) {
...
bool expand_layer_topdown_cpu_kernel3_mdd(MDD* mdd,
                                          const int l,
                                          ParetoFrontierManager* mgmr,
                                          const bool parallel_mode,
                                          const int threads) {
    ...
    for (int i = 0; i < layer_size; ++i) {
        ...
        for (MDDArc* arc : node->in_arcs_list) {
            ...
            const std::vector<ObjType>& prev_sols = arc->tail->pareto_frontier->sols;
            const size_t num_prev_sols = prev_sols.size() / NOBJS;
